#常用的网络，torch中都有这样的库如VGG，resNET，用于ID损失训练
...
class ResNet50(nn.Module):
    def __init__(self, num_classes, loss={'xent'}, **kwargs):#num_classes分类数和ID有关
        super(ResNet50, self).__init__()
        self.loss = loss
        resnet50 = torchvision.models.resnet50(pretrained=True)#pretrained=True使用预加载的参数，这样网络更易收敛
        self.base = nn.Sequential(*list(resnet50.children())[:-2])#不要restnet50的最后两行，自己添加下面两行

        self.classifier = nn.Linear(2048, num_classes)
        self.feat_dim = 2048 # feature dimension

    def forward(self, x):
        x = self.base(x)##输入一个256，128大小的随机张量，经过base后得到，x_torch.size([32,2048,8,4])

        x = F.avg_pool2d(x, x.size()[2:])#用一个pooling，将8，4大小的feature global pooling 成1，1
        #F.avg_pool2d(x, x.size()[2:])，输入第一个值为输入图片，第二个值为pooling大小
        f = x.view(x.size(0), -1)#x_torch.size([32,2048,1,1])， F_torch.size[32,2048]
        if not self.training:#训练阶段是要用FC来分类成概率，并生成loss，优化网络，测试阶段只要获取quary的特征，以及gallery的特征，然后检索就行
            return f
        y = self.classifier(f)#y.shape (32,751)

        if self.loss == {'xent'}:
            return y
        elif self.loss == {'xent', 'htri'}:#这里若采用交叉商和度量学习的话，既要返回经过Linear后的(32,751)，又要返回特征f
            return y, f
        elif self.loss == {'cent'}:
            return y, f
        elif self.loss == {'ring'}:
...
#     imgs=torch.Tensor(32,3,256,128)#输入一个256，128大小的随机张量
#     embed()
#     f=model(imgs)
