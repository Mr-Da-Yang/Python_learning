#可以跑
#Train image model with cross entropy loss and hard triplet loss，两个loss互相比拼，一个大一个就会小，最终两个loss加起来就会变小
#Datasets  第一行的data要变
...
from luohao_person_reid.losses import CrossEntropyLabelSmooth, TripletLoss, DeepSupervision
...
from luohao_person_reid.samplers import RandomIdentitySampler
...
parser.add_argument('--margin', type=float, default=0.3, help="margin for triplet loss")#margin
parser.add_argument('--num-instances', type=int, default=4,
                    help="number of instances per identity")#每个ID选4张图
parser.add_argument('--htri-only', action='store_true', default=False,
                    help="if this is True, only htri loss is used in training")#默认为False:即分类损失和triple一起用
# Architecture
...
def main():
    ...
    trainloader = DataLoader(
        ...
        sampler=RandomIdentitySampler(dataset.train, num_instances=args.num_instances),#args.num_instances一个ID4张图片，
    ...
    for batch_idx, (imgs, pids, _) in enumerate(trainloader):
        ...
        if args.htri_only:#只用三元损失
            ...
            if isinstance(outputs, tuple):
                ...
                xent_loss = criterion_xent(outputs, pids)#对于交叉熵损失，输入的是outputs(batchx751)
            
            if isinstance(features, tuple):
                htri_loss = DeepSupervision(criterion_htri, features, pids)
            else:
                htri_loss = criterion_htri(features, pids)#对于度量学习，输入的是特征batchx2048
